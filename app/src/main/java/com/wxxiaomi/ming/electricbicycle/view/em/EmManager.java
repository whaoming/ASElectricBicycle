...
public class EmManager {
				...
				.setContactListener(new EMContactListener() {
					...
					public void onContactAgreed(final String username) {
						...
						UserEngineImpl engine = new UserEngineImpl(
								EBApplication.getInstance()
										.getApplicationContext());
						engine.getUserCommonInfoByEmname(username,
								new ResultByGetDataListener<InitUserInfo>() {
									...
									public void success(
										...
										list.add(result.infos.friendList.get(0));
					...
					public void onContactInvited(final String username,
						...
						UserEngineImpl engine1 = new UserEngineImpl(
								EBApplication.getInstance()
										.getApplicationContext());
						engine1.getUserCommonInfoByEmname(username,
								new ResultByGetDataListener<InitUserInfo>() {
									@Override
									public void success(
											ReceiceData<InitUserInfo> result) {
										...
										tempUserDao.savaPerson(result.infos.friendList.get(0));
										if(friendReqListener!=null){
											friendReqListener.friendStatChange();
										}
										if(friendReqDetailListener!=null){
	 ...
	 * 同意好友申请
	 */
	public void agreeInvite(User.UserCommonInfo userInfo, EmResultCallBack<String> lis){
		try {
